...
import { Sparkles, Users, Search, BookOpen, Clock } from 'lucide-react';
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {portraits.map(([roleName, p]) => {
                    ...
                    const isExpanded = expandedId === `${profile.id}-${roleName}`;
                    ...
                    return (
                      <div 
                        ...
                        onClick={() => setExpandedId(isExpanded ? null : `${profile.id}-${roleName}`)}
                      >
                        ...
                        {isExpanded && (
                          <div className="mt-10 pt-10 border-t border-slate-100 flex justify-end">
                            <button className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-indigo-600 transition-colors shadow-lg shadow-slate-200">
                              收起详情
                            </button>
                          </div>
                        )}
...
function PortraitTag({ label, value, color, large }: { label: string, value: string, color: string, large?: boolean }) {
  ...
  return (
    <div className={`rounded-2xl border ${colors[color]} font-bold group/tag ${
...
function FeatureCard({ icon, text }: { icon: React.ReactNode, text: string }) {
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-sm font-bold">
      {icon}
      {text}
    </div>
  );
